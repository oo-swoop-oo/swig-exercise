...
## Examples
The following illustrate the SWIG target language specific workflows for building the shared libraries (or extensions) and running the test harness code for them.

### Javascript
From the repository root, start by building the shared library for Javascript (i.e. NodeJS extension):
```
$ script/javascript/build.sh
```
...
```
$ cd ..
$ script/javascript/clean.sh
```

### Java
From the repository root, start by building the shared library for Java:
```
